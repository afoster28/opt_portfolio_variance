...
public:
    ...
    void saveToCSV(const std::vector<double>& annualReturns,
        ...
        if (outputFile.is_open()) {
            ...
            outputFile << "annual_returns,log_returns,asset_prices";
            ...
            for (size_t i = 0; i < annualReturns.size(); ++i) {
                // Write annual_returns, log_returns, and asset_prices
                outputFile << std::fixed << std::setprecision(2) << annualReturns[i] << ','
                        << logReturns[i] << ',' << assetPrices[i];
    ...
    void runModel(int numReturns, double spotPrice) {
        ...
        // Combine and save data to a CSV file
        saveToCSV(returnList, logReturns, priceList, optionPayouts, optionsList, "combined_table.csv");
    }
};
